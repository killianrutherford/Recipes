...
def group_labels_z(file):
	data = pd.read_csv(file, sep=',')
	d = data.as_matrix(columns=["Zone1","Zone2","Zone3","Zone4","Zone5","Zone6","Zone7",
				"Zone8","Zone9","Zone10",
...
def load_temp(data, target):
	...
	return lasagne.utils.floatX(X_train), Y_train.astype('float32')
...
def main(n=5, num_epochs=100, model=None):
	...
	data = load_data_1("/mnt/disks/gpu-disk/stage1_labels_zt.csv")
	...
	if model is None:
		...
		updates = lasagne.updates.momentum(
				loss, params, learning_rate = sh_lr)
	...
	test_acc = T.mean(abs(T.round(test_prediction) - target_var), axis = 0, dtype=theano.config.floatX)
	...
	if model is None:
		...
		for epoch in range(num_epochs):
			...
			np.savez('weights_z/passenger_screening_' +str(t1) + '.npz', *lasagne.layers.get_all_param_values(network))
